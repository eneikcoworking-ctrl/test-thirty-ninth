<script lang="ts">
  ...
  // --- AI Settings State (Main Feature) ---
  ...
  let stopTriggers: StopTrigger[] = $state([]);
  ...
  // Active Tab: 'campaigns' | 'ai-tuning'
...
{#if activeTab === 'campaigns'}
  ...
  <div class="min-h-screen bg-background text-on-surface pb-24 md:pb-8 flex flex-col font-sans">
    ...
    <nav class="fixed bottom-0 w-full z-50 flex justify-around items-center px-4 py-2 bg-surface border-t border-outline-variant rounded-t-xl shadow-2xl">
      <button
        type="button"
        onclick={() => activeTab = 'campaigns'}
        class="flex flex-col items-center justify-center bg-primary-container text-on-primary-container rounded-full px-4 py-1 active:scale-90 duration-150 cursor-pointer"
      >
      ...
      <button
        type="button"
        onclick={() => activeTab = 'ai-tuning'}
        class="flex flex-col items-center justify-center text-on-surface-variant hover:bg-surface-container-highest transition-colors px-4 py-1 rounded-full cursor-pointer"
      >
...
{:else}
  <!-- AI Settings / Tuning View (from Main) -->
  <div class="min-h-screen bg-[#0A0A0B] text-[#e3e2e7] flex flex-col font-sans">
    <!-- TopAppBar -->
    ...
    <nav class="fixed bottom-0 w-full z-50 flex justify-around items-center px-4 py-2 pb-safe bg-[#121317]/80 backdrop-blur-xl border-t border-white/10" aria-label="Bottom Navigation">
      <button
        type="button"
        onclick={() => activeTab = 'campaigns'}
        class="flex flex-col items-center justify-center text-gray-400 hover:text-[#adc6ff] transition-all active:scale-90 p-1 rounded focus:outline-none focus-visible:text-[#adc6ff]"
      >
        <span class="material-symbols-outlined">dashboard</span>
        <span class="font-mono text-[10px] tracking-wider uppercase mt-0.5">Campaigns</span>
      </button>
      <button
        type="button"
        onclick={() => activeTab = 'ai-tuning'}
        class="flex flex-col items-center justify-center text-[#00285c] bg-[#adc6ff] rounded-full px-4 py-1 active:scale-90 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
      >
        <span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">temp_preferences_custom</span>
        <span class="font-mono text-[10px] tracking-wider uppercase mt-0.5">AI Persona</span>
      </button>
    </nav>
  </div>
{/if}
...
<style>
  ...
  /* Shared styles for Highlighter/AI settings from main */
  .glass-card {
  ...
  .highlighter-textarea, .highlighter-overlay {
    position: absolute;
    top: 0;
    left: 0;
    width: 100%;
